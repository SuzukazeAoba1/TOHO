        ...
        //if (!pause)
        {
            if (spawnCounter < SpawnContainer.SpawnContainer.Count)
            {
                ...
                if (nextSpawnTime < spawnTimer)
                {
                    ...
                    buf.GetComponent<Enemy>().SetBarrageSetting(BarrageList,
                                                                BarrageSequence.List[spawnbuf.m_barragesequenceid], 
                                                                BarragePatten.List[pattenid], 
                                                                spawnbuf.m_spawnfilp, 
                                                                BarrageSequence.List[spawnbuf.m_barragesequenceid].m_shotinterval);
                    spawnCounter += 1;
                }
            }
        }
